...
The first rucksack contains the items vJrwpWtwJgWrhcsFMMfFFhFp, 
    its first compartment contains the items vJrwpWtwJgWr, 
...
def split_bag(backpack: list) -> list:
    half_point = int(len(backpack)/2)
    return backpack[0:half_point], backpack[half_point:]

def get_common(cp1: list, cp2: list, cp3=False) -> list:
    if not cp3:
        return list(set([v for v in cp1 if v in cp2]))
    else:
        return list(set([v for v in cp1 if v in cp2 and v in cp3]))
...
def part_one():
    ...
    for backpack in backpacks:
        compartment_1, compartment_2 = split_bag(backpack)
        common = get_common(compartment_1, compartment_2)
        for v in common:
...
backpacks = aoc_utils.read_file('data/d3.txt', listify=True)
