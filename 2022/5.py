...
def move_one_by_one(crates: list, step: list) -> list[list]:
...
def move_together(crates: list, step: list) -> list[list]:
    crates_to_move, from_stack, to_stack = step
    crates_to_move = 0-crates_to_move
    crate_val = crates[from_stack][crates_to_move:]
    crates[to_stack]+=crate_val
    crates[from_stack] = crates[from_stack][0:crates_to_move]
    return crates

def part_one():
    crates, steps = get_start()
    for step in steps:
        crates = move_one_by_one(crates, step)
...
def part_two():
    crates, steps = get_start()
    for step in steps:
        crates = move_together(crates, step)
    print(''.join([c[-1] for c in crates]))
    return
